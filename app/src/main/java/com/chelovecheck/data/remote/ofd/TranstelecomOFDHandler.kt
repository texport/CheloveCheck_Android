...
class TranstelecomOFDHandler @Inject constructor(
    ...
    override suspend fun fetchReceipt(url: String): Receipt {
        logDebug("fetchReceipt start, url=$url")
        val response = try {
            httpClient.get(url, headers = defaultHtmlHeaders())
        ...
        throwIfHttpError(response.code)
    ...
    private suspend fun fetchAfterCaptchaOrThrow(originalUrl: String): Receipt {
        ...
        val retry = try {
            httpClient.get(fallbackUrl, headers = xhrFragmentHeaders())
        } catch (error: Throwable) {
        ...
        return runCatching { parseTicketDocument(Jsoup.parse(retry.body), originalUrl) }
            .getOrElse { error ->
                logger.error(TAG, "fallback parse error", error)
                throw AppError.ReceiptRequiresOfdVerification(originalUrl)
            }
    }
    ...
    override suspend fun fetchReceiptWithCaptchaToken(url: String, captchaToken: String): Receipt {
        ...
        val response = try {
            httpClient.get(built, headers = xhrFragmentHeaders())
        } catch (error: Throwable) {
            logger.error(TAG, "captcha token request failed", error)
            throw AppError.NetworkError(error)
        }
    ...
    private fun throwIfHttpError(code: Int) {
        if (code == 404) {
            throw AppError.ReceiptNotFound
        }
        if (code != 200) {
            throw AppError.NetworkError(IllegalStateException("HTTP $code"))
        }
    }
